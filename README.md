...
| 🔌 | 通过只读 SNMP v1/v2c/v3 获取受管交换机、路由器和 AP 的 LLDP 邻居与端口信息 |
...
如果路由器、交换机或无线 AP 带管理功能，可以进入“系统设置 → 扫描设置 → 受管网络设备与 LLDP”添加它。MeowTopo 支持：

- SNMP v1 和 v2c 的只读 Community
- SNMP v3 的 `noAuthNoPriv`、`authNoPriv` 和 `authPriv`
- MD5、SHA、SHA-224、SHA-256、SHA-384、SHA-512 认证
- DES、AES、AES-192、AES-256 加密，并兼容常见厂商使用的两种 AES 密钥扩展方式
- 标准 LLDP-MIB 中的邻居机箱标识、系统名称、本地端口、远端端口和端口描述

建议优先使用 SNMP v3 的“认证并加密”；只支持 v2c 的设备应创建专用只读 Community，并通过设备访问控制列表仅允许 MeowTopo 所在主机访问。不要填写具有写入权限的凭据。

添加后可以先点“测试”确认认证信息，再点“立即读取全部设备”。启用的受管设备也会在常规网络扫描完成后自动读取。网页只显示认证信息是否已经配置，不会把保存的 Community 或密码重新返回浏览器。

LLDP 连接会标记为设备协议发现并显示端口名称。系统只会连接已经被基础扫描、手工创建或其他方式识别到的设备，不会仅凭一个无法核实的邻居名称生成大量虚假节点。用户确认过的连接始终优先，LLDP 不会覆盖它。部分设备虽然支持 SNMP，但没有启用 LLDP，界面会明确显示“SNMP 正常，LLDP 不可用”。
...
如果你把自动识别的设备类型改成其他类型，MeowTopo 会保留当时的自动结果、判断依据和你的修正；后续扫描不会覆盖你的选择。这些记录只保存在本地数据库中。同一厂商至少两台设备存在有效修正，且其中至少四分之三选择相同类型时，MeowTopo 才会把它作为较弱的本地识别线索；取消手工类型后，对应修正不再参与学习。
...
普通家用路由器和傻瓜交换机通常不会提供端口转发表、LLDP、SNMP 或控制器数据。缺少这些信息时，MeowTopo 展示的是方便整理设备的**逻辑关系**，不是真实网线和交换机端口的完整还原。支持 SNMP 与 LLDP 的受管设备可以提供可信度更高的邻居和端口关系。
...
数据库包含家庭设备信息、设置、账户密码摘要、登录记录以及已配置的 SNMP 认证信息。不要将 `data` 目录提交到 GitHub，也不要公开分享备份。
...
go test ./...
go vet ./...
Get-ChildItem internal/app/web/*.js | ForEach-Object { node --check $_.FullName }
go build -trimpath -ldflags="-s -w" -o meowtopo ./cmd/meowtopo
...
MeowTopo 目前处于可部署试用阶段。后续方向包括更多通知渠道、更准确的设备识别、更多厂商控制器数据来源、更完整的手机端体验，以及持续完善 Topo酱视觉细节。
