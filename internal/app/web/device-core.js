...
function openDetail(id) {
  ...
  const conn = connections.find(
    (c) => (c.TargetID || c.target_device_id) === id,
  );
  const parentId = conn ? conn.SourceID || conn.source_device_id : 0;
  const connType = conn ? conn.Type || conn.connection_type : "unknown";
  const connPort = conn ? conn.Port || conn.port_label : "";
  ...
  const connOptions = ["unknown", "ethernet", "wifi", "logical", "virtual"]
    .map(
      (v) =>
        `<option value="${v}" ${v === connType ? "selected" : ""}>${{ unknown: "未知", ethernet: "网线", wifi: "Wi-Fi", logical: "逻辑连接", virtual: "虚拟连接" }[v]}</option>`,
    )
    .join("");
  $("#detail").innerHTML =
...
async function saveDetail(d) {
  if (appExtensions.saveDeviceDetail)
    return appExtensions.saveDeviceDetail(d);
  try {
    await api(`/api/devices/${d.id}`, {
      method: "PATCH",
      body: JSON.stringify({
        user_name: $("#editName").value,
        user_device_type: $("#editType").value,
        notes: $("#editNotes").value,
        is_new: false,
        is_ignored: $("#editIgnored").checked,
        always_show: $("#editAlways").checked,
      }),
    });
    await api(`/api/devices/${d.id}/connection`, {
      method: "PATCH",
      body: JSON.stringify({
        parent_id: +$("#editParent").value,
        connection_type: $("#editConn").value,
        port_label: $("#editPort").value,
      }),
    });
    await refresh();
    openDetail(d.id);
